% Generate MATLAB rASRMatlab reference outputs for mne-denoise.
%
% Prerequisites:
%   1. Run generate_asr_input.py from the repository root.
%   2. Add rASRMatlab/rASRToolbox and its Manopt dependency to the path.
%
% Outputs:
...
if exist('clean_windows', 'file') ~= 2
    error('rASRMatlab clean_windows.m is not on the MATLAB path.');
end
if exist('asr_calibrate', 'file') ~= 2
    error('rASRMatlab asr_calibrate.m is not on the MATLAB path.');
end
if exist('asr_process', 'file') ~= 2
    error('rASRMatlab asr_process.m is not on the MATLAB path.');
end
if exist('rasr_nonlinear_eigenspace', 'file') ~= 2
    error('rASRMatlab rasr_nonlinear_eigenspace.m is not on the MATLAB path.');
end

inputs = dir(fullfile(here, 'asr_case_input_*.mat'));
if isempty(inputs)
    error('No ASR input cases found. Run generate_asr_input.py first.');
end
...
for idx = 1:numel(inputs)
    ...
    output_path = fullfile(here, sprintf('rasr_case_reference_%s.mat', case_name));
...
% The audited rASRMatlab implementation has the original ten-argument
% calibration signature and therefore does not accept maxmem here.
state = asr_calibrate( ...
